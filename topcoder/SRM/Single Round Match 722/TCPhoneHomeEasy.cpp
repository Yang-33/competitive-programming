...
/* -----  xtimex  Problem: SRM 722 div2 med TCPhoneHomeEasy / Link:  ----- */
/* ------問題------

10^digitの電話番号がある。
特別なprefixのものは使えない。有効な電話番号は？
ただし重複しない。
...
重複しないのでね、全部消していいよね
重複する場合はsortして部分文字列が重複しているものを取り除いてから同様の処理をすればよい。
...
class TCPhoneHomeEasy 
{
    ...
    int validNumbers(int digits, vector<string> specialPrefixes) 
	{
		...
		FOR(i, 0, SZ(specialPrefixes)) {
			int len = SZ(specialPrefixes[i]);
			LL sub = pow(10, digits - len);
			sum -= sub;
...
struct TCPhoneHomeEasy {
  ...
  int validNumbers(int _digits, vector<string> _specialPrefixes) {
