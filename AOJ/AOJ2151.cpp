...
/* -----  2018/06/10  Problem: AOJ 2151 / Link: http://judge.u-aizu.ac.jp/onlinejudge/description.jsp?id=2151  ----- */
/* ------問題------
...
ダイクストラで頂点を区別できればいいので状態を拡張する。

----解説ここまで---- */

using tp = tuple<int, int, int>;

VVL dijkstra(vector<vector<tp>>& G, int L) {
	int N = SZ(G);
	VVL dist(N, VL(L + 1, 1e16));

	priority_queue<tp, vector<tp>, greater<tp>>pq;
	pq.push(tp(0, 0, L));
	dist[0][L] = 0; // 残りLえん持っているときの襲われる最小の回数
