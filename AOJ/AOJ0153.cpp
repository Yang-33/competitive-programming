...
/* -----  2019/07/23  Problem: AOJ 0153 / Link: https://onlinejudge.u-aizu.ac.jp/challenges/search/volumes/0153  ----- */
...
int main() {
	...
	while (cin >> x >> y, x || y) {
		...
		if (circle_in_poly(g, c)) {
			puts("a");
		}
		else if (poly_in_circle(c, g)) {
			puts("b");
		}
		else if (is_cross_circle_and_polygon(c, g)) {
			puts("c");
		}
		else {
			puts("d");
