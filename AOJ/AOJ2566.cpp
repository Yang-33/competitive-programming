...
/* -----  2019/05/26  Problem: AOJ 2566 / Link: https://onlinejudge.u-aizu.ac.jp/challenges/search/volumes/2566  ----- */
...
int main() {
	...
	while (1) {
		...
		FOR(i, 0, SZ(a))if (isdigit(a[i]))a[i] -= '0';
		FOR(i, 0, SZ(b))if (isdigit(b[i]))b[i] -= '0';
		FOR(i, 0, SZ(c))if (isdigit(c[i]))c[i] -= '0';

		int n = max({ SZ(a),SZ(b),SZ(c) });
		vector<vector<mint>>dp(n + 1, vector<mint>(2));
		auto rangeset = [](char c) {
			if (c == '?')return PII(0, 10);
			return PII(c, c + 1);
		};
		dp[0][0] = 1;
		FOR(i, 0, n) {
			FOR(carry, 0, 2) {
				if (dp[i][carry].get() == 0)continue;
				PII ar = rangeset(a[i]), br = rangeset(b[i]), cr = rangeset(c[i]);
				FOR(ai, ar.first, ar.second) {
					FOR(bi, br.first, br.second) {
						int sum = ai + bi + carry;
						int low = sum % 10;
						int nxcarry = sum / 10;
						if (cr.first <= low && low < cr.second) {
							// leading zero
							if (i + 1 == n && (ai == 0 || bi == 0 || low == 0))continue;
							dp[i + 1][nxcarry] += dp[i][carry];
						}
					}
				}
