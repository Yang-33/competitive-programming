...
/* -----  2019/08/04  Problem: AOJ 0214 / Link: https://onlinejudge.u-aizu.ac.jp/challenges/search/volumes/0214  ----- */
...
int main() {
	...
	while (cin >> N, N) {
		FOR(_, 0, N) {
			...
			FOR(i, 0, M) {
				FOR(j, 0, i) {
					if (poly_touch_poly(ps[i], ps[j]) != OUT) {
						uf.unionSet(i, j);
					}
				}
			}
