...
/* -----  2019/07/21  Problem: AOJ 0143 / Link: https://onlinejudge.u-aizu.ac.jp/challenges/search/volumes/0143  ----- */
...
int main() {
	...
	FOR(_, 0, N) {
		Polygon poly(0); {
			FOR(i, 0, 3) {
				double x, y; cin >> x >> y;
				poly.vert.emplace_back(x, y);
			}
		}
		double x, y; cin >> x >> y;
		Point s(x, y); cin >> x >> y;
		Point t(x, y);
		bool shadan = (point_in_poly(poly, s) == OUT) ^ (point_in_poly(poly, t) == OUT);
		cout << (shadan ? "OK" : "NG") << "\n";
	}
}