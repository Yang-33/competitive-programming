...
/* -----  2019/07/15  Problem: HUPC2019day2 G / Link: __CONTEST_URL__  ----- */
...
int main() {
	...
	vector<vector<mint>>use(N, vector<mint>(K + 1, 0));
	vector<vector<mint>>nouse(N, vector<mint>(K + 1, 0));
	VI sz(N);
	function<void(int, int)>f = [&](int v, int p) {
		...
		for (auto nx : G[v]) {
			if (nx == p)continue;
			f(nx, v);
			vector<mint>nxuse(sz[v] + sz[nx] + 1);
			vector<mint>nxnouse(sz[v] + sz[nx] + 1);
			FOR(a, 0, sz[v] + 1) {
				FOR(b, 0, sz[nx] + 1) {
					int s = a + b;
					// LINK
					{
						if (s)nxuse[s - 1] += use[v][a] * use[nx][b];
					}

					// CUT
					{
						nxuse[s] += use[v][a] * use[nx][b];
						nxuse[s] += use[v][a] * nouse[nx][b];
						nxnouse[s] += nouse[v][a] * use[nx][b];
						nxnouse[s] += nouse[v][a] * nouse[nx][b];
					}
				}
			}
			sz[v] += sz[nx];
			nxuse.swap(use[v]);
			nxnouse.swap(nouse[v]);
			if (sz[v] > K) {
				sz[v] = K;
			}
