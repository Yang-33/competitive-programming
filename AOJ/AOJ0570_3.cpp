...
LL f(string &target, int i, int m, int bef, int ud, int less) {
	...
	if (dp[i][m][bef][ud][less] != -1&&less==1)return  dp[i][m][bef][ud][less];
	LL res = 0;
	int num = target[i] - '0';
	int Max = less ? 9 : num;
	...
	if (less == 1)return  dp[i][m][bef][ud][less] = res;
	return res;
}
...
int main() {
	...
	ret1 += f(B, SZ(B)-1, 0, 0, 0, 0);
	//debug(ret1);
	if (A != "0") {
		ret2 = f(A, SZ(A)-1, 0, 0, 0, 0);
	}
	//debug(ret2);
	ans = ret1 - ret2;
	ans = (ans + 30000) % 10000;
