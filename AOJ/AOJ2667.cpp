...
/* -----  2018/07/05  Problem: AOJ 2667 / Link: https://onlinejudge.u-aizu.ac.jp/challenges/search/volumes/2667  ----- */
/* ------問題------
...
HeavyPathを先頭に持っていくと、性質を保ったままオイラツアーができるので、
HLDで部分木も扱える。
...
struct LazySegTreeP {
	...
	void lazy_push(ll k, ll queryL, ll queryR) { // node⊕lazy
		...
		dat[k].val += (queryR-queryL)*lazy[k].lazyval; //値の塗りつぶし += とかかも
		if (k < N - 1) {
...
// subtree vertex
// seg.query(ps[v],pt[v],X);
//
// subtree edge
// seg.query(ps[v]+1,pt[v],X)
//
...
int main() {
	...
	FOR(q, 0, Q) {
		...
		if (a == 0) { //repo
			LL sum = 0;
			hld.for_each_edge(b, c, [&](int l,int r) {
				sum += seg.query(l,r+1).val; 
			});
			cout << sum << endl;
		}
		else { //add 

			seg.lazy_update(hld.ps[b]+1,hld.pt[b],c);
		}
	}
