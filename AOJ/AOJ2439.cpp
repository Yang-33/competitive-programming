...
/* -----  2019/07/21  Problem: AOJ 2439 / Link: https://onlinejudge.u-aizu.ac.jp/challenges/search/volumes/2439  ----- */
	...
	FOR(i, 0, N) {
		FOR(j, 0, i + 1) {
			if (cs[i] == '-') { // TIE
				...
				dp[i + 1][j - 1 + 1] += dp[i][j] * j; // 左決定
				dp[i + 1][j + 1] += dp[i][j];

			}
			else { // DOWN
				int nj = j - 1;
				if (j)dp[i + 1][nj] += dp[i][j] * j*j;
				dp[i + 1][j] += dp[i][j] * j; // 左保留
			}
		}
	}
	LL ans = dp[N][0].get();
