...
/* -----  2018/06/24  Problem: AOJ 2235 / Link: http://judge.u-aizu.ac.jp/onlinejudge/description.jsp?id=2235  ----- */
/* ------問題------

Dynamic Connectivity を実装しろ。ただし条件は以下
正しい入力はない
treeの入力しか来ない
V=40000,Q=40000
...
まず無視をするとダイコネを貼ればいいことがわかる。おしまい
想定解はundoUF+segtreeらしい(謎)
...
int main() {
	{
		...
		while (cin >> N >> Q) {
			...
			FOR(q, 0, Q) {
				...
				if (id == 1) {// add edge
					edgeMap[PII(a, b)] = q;
					fdc.insertEdge(q, a, b);
				}
				else if (id == 2) { // delete edge
					int manageID = edgeMap[PII(a, b)];
					fdc.deleteEdge(manageID);
					edgeMap.erase(PII(a, b));
				}
				else { // check
					if (fdc.isConnected(a, b)) {
						cout << "YES" << endl;
					}
					else {
						cout << "NO" << endl;
