...
/* -----  2019/07/19  Problem: AOJ 0090 / Link: https://onlinejudge.u-aizu.ac.jp/challenges/search/volumes/0090  ----- */
...
int main() {
	...
	while (cin >> N, N) {
		...
		FOR(i, 0, N) {
			...
			cs.emplace_back(Circle(Point(x, y), 1));
		}
		vector<Point>event_point;
		FOR(i, 0, N) {
			FOR(j, i + 1, N) {
				for (auto&it : cross_point(cs[i], cs[j])) {
					event_point.emplace_back(it);
				}
			}
		}
		LL ans = 1; // ここでWAした
		for (auto &p : event_point) {
			LL ret = 0;
			for (auto &c : cs) {
				ret += circle_in(c, p);
			}
