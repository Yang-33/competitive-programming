...
/* -----  2018/11/15  Problem: CodeForces520 C div2 / Link: http://codeforces.com/contest/1062/problem/C  ----- */
/* ------問題------
...
int main() {
	...
	FOR(_, 0, Q) {
		int l, r; cin >> l >> r;
		// [l,r]
		LL one = csum[r] - csum[l - 1];
		LL zero = r - l + 1 - one;
		mint asum = two.pow(one) - ONE;
		mint bsum = two.pow(zero) - ONE;
		mint ans = asum + asum * bsum;
		DD(de(one, zero,asum.get(),bsum.get()));
		cout << ans.get() << "\n";
	}
