...
/* -----  2019/02/27  Problem: ABC 113 D / Link: http://abc113.contest.atcoder.jp/tasks/abc113_d  ----- */
/* ------問題------
...
int main() {
	...
	FOR(i, 0, H) {
		...
		FOR(state, 0, 1 << (W - 1)) {
			bool valid = 1;
			FOR(k, 0, W - 2) {
				if ((state & 1 << k) && (state & 1 << (k + 1))) valid = false;
			}
			if (!valid)continue;
			FOR(k, 0, W) {
				if (state & 1 << k) {
