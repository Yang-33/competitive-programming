...
/* -----  2019/04/05  Problem: ABC 036 D / Link: http://abc036.contest.atcoder.jp/tasks/abc036_d  ----- */
...
int main() {
	...
	fill(*memo, *memo + 100005 * 2, -1);
	const int WHITE = 0;
	const int BLACK = 1;
	// black-black x.
	function<mint(int, int, bool)> f = [&](int v, int p, bool c) {
		if (memo[v][c] != -1)return mint(memo[v][c]);
