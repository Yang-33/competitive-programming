...
/* -----  2018/01/27  Problem: soundhound2018_spring C / Link: https://soundhound2018.contest.atcoder.jp/tasks/soundhound2018_c  ----- */
/* ------問題------
...
最大独立点集合を求めたい。
二部グラフにおいてはこれは高速に解けて、V - |MaxFlow|なのでできる。
...
int main() {
	...
	FOR(i, 0, H) {
		cin >> a[i];
		FOR(j, 0, W) {
			if (a[i][j] == '.')Map[PII(i,j)] = V++;
		}
	}
	Bipartite_Matching F(V);
	FOR(i, 0, H) {
		FOR(j, 0, W) {
			if (a[i][j] == '.') {
				FOR(k, 0, 4) {
					...
					if (0 <= ny&&ny < H && 0 <= nx&&nx < W) {
						if (a[ny][nx] == '.')F.add_edge(Map[PII(i,j)], Map[PII(ny,nx)]);
					}
				}
			}
		}
