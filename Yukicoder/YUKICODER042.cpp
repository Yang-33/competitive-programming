...
[証明]
dp[x][a*c[x]+b]は、aのx次式で表せる。これを示す。
dp[0][any]=1より、これは0次式
dp[x-1][any]がx-1次式のとき、dp[x][any]がx次式なら嬉しい。(帰納法が成り立つので！)

dp[x][a*C[x]+b] = dp[x][(a-1)*C[x]+b] + dp[x-1][a*C[x]+b]
dp[x][(a-1)*C[x]+b] = dp[x][(a-2)*C[x]+b] + dp[x-1][(a-1)*C[x]+b]
dp[x][(a-2)*C[x]+b] = dp[x][(a-3)*C[x]+b] + dp[x-1][(a-2)*C[x]+b]
...
dp[x][C[x]+b] = dp[x][b] + dp[x-1][C[x]+b]

となるので, 
dp[x][a*C[x]+b] = dp[x][b] + dp[x-1][a*C[x]+b] + dp[x-1][(a-1)*C[x]+b] + ... + dp[x-1][C[x]+b] …(☆)
...
ここで (☆) 式に戻ると, 右辺は 定数 + (a に関する x-1 次式 a 個の和) という形になっています。よって, 右辺は x 次式です。
以上より dp[x][a*C[x]+b] は x 次式になります。

(n付き二項間漸化式を考えるととても良い)
したがって、seedとなるのはbの部分。
これをもとに、6点を使用して関数を補完すれば正しい。


M= a*500+b 
calculate ans(t) using ans(0)=dp[0*500+b], ans(1)=dp[1*500+b],... ans(5)=dp[5*500+b]
